    ...
    function onClick() {
        ...
        function getRestaurants(input) {
            ...
            input.forEach(record => {
                ...
                const workers = workersList.split(', ').map(w => {
            ...
            Object.values(restaurants).forEach(r => {
                ...
                r.avgSalary = r.workers.reduce((sum, w) => sum + w.salary, 0) / r.workers.length;
            });
        ...
        function getBestRestaurant(restaurants) {
            return Object.entries(restaurants)
                .map(r => ({ name: r[0], avgSalary: r[1].avgSalary, workers: r[1].workers, number: r[1].number }))
